...
pub fn Sidebar(
    ...
    rsx! {
        button {
            class: format!(
                "sidebar-toggle bg-{} hover:bg-{} hover:transition-all hidden {}",
                theme.prim_200,
                theme.prim_300,
                if sidebar_hidden() { "collapsed" } else { "" },
            ),
            style: format!(
                "position: absolute; {}: {}rem;",
                if sidebar_left() { "left" } else { "right" },
                if sidebar_hidden() { "0" } else { "15" },
            ),
            onclick: move |_| {
        ...
        div {
            class: format!(
                "bg-{} snap-end max-h-screen hidden overflow-y-auto no-scrollbar {}",
                theme.prim_100,
                if sidebar_hidden() { "hidden" } else { "md:block md:w-60" },
            ),
            nav {
                div { class: "flex-1 grid items-start py-2 text-sm font-medium",
                            ...
                            button {
                                ...
                                onclick: move |_| {
                                    match bible() {
                                        ...
                                        None => {
                                            debug!("Failed to load Bible book: {}", book);
                                        }
                                    }
                                },
                                if curr_bible.get_current_chapter().map_or(false, |chapter| chapter.book == book) {
                                    div {
                                        class: format!(
                                            "flex justify-between align-middle text-base p{}-4 text-white bg-{}",
                                            sidebar_padding_side,
                                            theme.prim_700,
                                        ),
                                        if sidebar_left() {
                                            strong { class: "flex items-center", "{book.to_uppercase()}" }
                                        }
                                        input {
                                            class: format!(
                                                "rounded-{}-lg w-14 m{}-4 px-2 py-2 cursor-pointer {} text-right bg-{} appearance-none outline-bg-{}",
                                                sidebar_padding_side,
                                                sidebar_padding_side,
                                                theme.text,
                                                theme.prim_300,
                                            ...
                                            onchange: move |evt| {
                                                if let Some(temp_bible) = bible() {
                                                    ...
                                                    let chapter_num = match chapter_num {
                                                        ...
                                                        num if num > num_chapters_in_book => {
                                                            entered_chapter_num.set(1.to_string());
                                                            num_chapters_in_book
                                                        }
                                                        num => {
                                                            entered_chapter_num.set(1.to_string());
                                                            num
                                                    ...
                                                    let current_chapter_ref = temp_bible
                                                        .get_current_chapter()
                                                        .map_or("".to_string(), |chapter| chapter.r#ref.clone());
                                        ...
                                        if !sidebar_left() {
                                            strong { class: "flex items-center pr-4",
                                                "{book.to_uppercase()}"
                                            }
                                        }
                                    }
                                } else {
                                    div { class: format!("py-2 hover:bg-{} text-{}", theme.prim_300, theme.prim_900),
                                        "{book}"
                                    }
                                }
                            }
